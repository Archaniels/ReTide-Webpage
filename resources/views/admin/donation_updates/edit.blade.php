...
<div class="min-h-screen bg-gray-50 py-12">
    <div class="max-w-2xl mx-auto px-6">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Edit Update Donasi</h1>
            <p class="text-gray-600 mt-2">Update ID: #{{ $update->id }}</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white rounded-lg shadow-lg p-8">
            <form method="POST" action="{{ route('admin.donations.updates.update', [$donation->id, $update->id]) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="description" name="description"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="Masukkan deskripsi update" rows="5" required>{{ old('description', $update->description) }}</textarea>
                ...
                <div>
                    ...
                    <select id="status" name="status"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            required>
                        <option value="pending" @selected(old('status', $update->status) == 'pending')>⏳ Pending</option>
                        <option value="in_progress" @selected(old('status', $update->status) == 'in_progress')>🔄 In Progress</option>
                        <option value="completed" @selected(old('status', $update->status) == 'completed')>✅ Completed</option>
                    </select>
